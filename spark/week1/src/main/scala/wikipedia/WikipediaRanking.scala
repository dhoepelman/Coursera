...
object WikipediaRanking {
  ...
  private def articleContainsLang(lang: String, article: WikipediaArticle) = article.text.split(' ').contains(lang)
   ...
   */
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = rdd.filter(articleContainsLang(lang, _)).count().toInt
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] = rdd.flatMap(article => {
    langs.collect({
      case lang if articleContainsLang(lang, article) => lang -> Iterable(article)
    })
  }).reduceByKey(_ ++ _)
  ...
  def rankLangsReduceByKey(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = rdd.flatMap(article =>
    langs.collect({
      case lang if articleContainsLang(lang, article) => lang -> 1
    })
